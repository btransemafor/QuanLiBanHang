...
WHERE HD.SOHD IN (SELECT TOP 1 WITH TIES  SOHD FROM HOADON HD 
WHERE  YEAR(NGHD) = 2006 
GROUP BY SOHD , trigia 
ORDER BY TRIGIA DESC )
...
SELECT HD.MAKH , KH.HOTEN ,COUNT(SOHD) 
FROM HOADON HD 
INNER JOIN 
(SELECT TOP 10 MAKH ,HOTEN 
FROM KHACHHANG 
...
ON KH.MAKH = HD.MAKH 
GROUP BY HD.MAKH , KH.HOTEN 
HAVING COUNT(HD.SOHD) >= all (SELECT COUNT(HD2.SOHD) FROM HOADON HD2
						      WHERE HD2.MAKH IN (SELECT TOP 10 MAKH FROM KHACHHANG ORDER BY DOANHSO DESC) 
						      GROUP BY HD2.MAKH ) 
